...
class WargaImport implements ToCollection
{
        ...
        foreach ($rows as $key => $row) {
            ...
            if ($key > 5) {
                ...
                try {
                    ...
                    // array_push($data_arr, $no_urut);

                } catch (\Throwable $th) {
                    // dd()
                    // throw $th;
                    // Log::error($th->getMessage());
                    Log::error("Row ke-{$key} gagal: " . $th->getMessage(), ['row_data' => $row]);
                }
            }
        }
    ...
    {
        $tgl_lahir_convert = $this->transformDate($tanggal_lahir);
        $now = Carbon::now();
        $usia = $now->diffInYears($tgl_lahir_convert);
        return $usia;
    }
}
